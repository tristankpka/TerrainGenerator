...
Terrain::Terrain()
  {
    ...
    std::mt19937 rng(rd()); 
    std::uniform_real_distribution<float> uni(0, 1.0f); // guaranteed unbiased
    
    m_vertices.reserve(TERRAIN_SIZE * TERRAIN_SIZE);
    for(int x = 0; x < TERRAIN_SIZE; x++)
      for(int y = 0; y < TERRAIN_SIZE; y++)
    	m_vertices.push_back(Vertex(glm::vec3(x * TERRAIN_TILE_SIZE, y * TERRAIN_TILE_SIZE, uni(rng)), glm::vec2(0, 0), glm::vec3(0, 0, 0)));
